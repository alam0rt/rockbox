...
# YP3Box codec XIP split: each codec's .text and .rodata are linked into its own
# SPI NOR slot instead of being copied into codecbuf, which then only has to
# hold .data and .bss. The generated file carries one target-specific
# CODECLDFLAGS line per codec giving that codec's slot base; plugin.lds places
# .codecxip there. Generated by tools/gen_codec_slots.py in the yp3box repo and
# written into the build directory by tools/40_build.sh - included, not
# required, so a build without it still links the ordinary way.
ifeq ($(MODELNAME),yp3box)
-include $(BUILDDIR)/codec-slots.mk
endif
